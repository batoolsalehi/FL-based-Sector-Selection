...
def get_data(data_paths,modality,key,test_on_all,path_test_all):   # per cat for now, need to add per epside for FL part
    ...
    print('test set shape after:',test_data.shape)
...
parser.add_argument('--model_folder', help='Location of the trained models folder', type=str,default = '/home/batool/FL/baseline_code/model_per_episode/')
...
episodes_involved = '_'.join(args.experiment_epiosdes)   #model structure is the same for all episode combinations but the weights are diffrent
...
print('RF data shapes',RF_train.shape,RF_val.shape,RF_test.shape)
    ...
    if 'coord' in args.input and 'lidar' in args.input:
        ...
        model.compile(loss=categorical_crossentropy,
                      ...
                      metrics=[metrics.categorical_accuracy,
                                        ...
                                        top_50_accuracy])
        model.summary()
        hist = model.fit(x_train, y_train, validation_data=(x_validation, y_validation), epochs=args.epochs,batch_size=args.bs, callbacks=[tf.keras.callbacks.ModelCheckpoint(args.model_folder+'best_weights.coord_lidar_'+episodes_involved+'.h5', monitor='val_loss', verbose=1, save_best_only=True,mode='auto'),tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=25, verbose=2,mode='auto')])
        ...
        model.load_weights(args.model_folder+'best_weights.coord_lidar_'+episodes_involved+'.h5', by_name=True)
...
###############################################################################
        ...
        hist = model.fit(x_train, y_train,validation_data=(x_validation, y_validation), epochs=args.epochs, batch_size=args.bs, callbacks=[tf.keras.callbacks.ModelCheckpoint(args.model_folder+'best_weights.coord_img_'+args.image_feature_to_use+'_'+episodes_involved+'.h5', monitor='val_loss', verbose=1, save_best_only=True,mode='auto'),tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=25, verbose=2, mode='auto')])
        ...
        print('***************Testing the model************')
        model.load_weights(args.model_folder+'best_weights.coord_img_'+args.image_feature_to_use+'_'+episodes_involved+'.h5', by_name=True)
        scores = model.evaluate(x_test, y_test)
        print(model.metrics_names, scores)
...
###############################################################################
        ...
        hist = model.fit(x_train, y_train, validation_data=(x_validation, y_validation), epochs=args.epochs,batch_size=args.bs, callbacks=[tf.keras.callbacks.ModelCheckpoint(args.model_folder+'best_weights.img_lidar_'+args.image_feature_to_use+'_'+episodes_involved+'.h5', monitor='val_loss', verbose=1, save_best_only=True,mode='auto'),tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=25, verbose=2,mode='auto')])
        ...
        print('***************Testing the model************')
        model.load_weights(args.model_folder+'best_weights.img_lidar'+args.image_feature_to_use+'_'+episodes_involved+'.h5', by_name=True)
        scores = model.evaluate(x_test, y_test)
        print(model.metrics_names, scores)
...
###############################################################################
    ...
    hist = model.fit(x_train, y_train, validation_data=(x_validation, y_validation), epochs=args.epochs, batch_size=args.bs, callbacks=[tf.keras.callbacks.ModelCheckpoint(args.model_folder+'best_weights.coord_img_lidar_'+args.image_feature_to_use+'_'+episodes_involved+'.h5', monitor='val_loss', verbose=1, save_best_only=True,mode='auto'),tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=25, verbose=2,mode='auto')])
    ...
    print('***************Testing the model************')
    model.load_weights(args.model_folder+'best_weights.coord_img_lidar_'+args.image_feature_to_use+'_'+episodes_involved+'.h5', by_name=True)
    scores = model.evaluate(x_test, y_test)
    print(model.metrics_names, scores)
...
###############################################################################
    ...
    if 'coord' in args.input:
        ...
        if args.strategy == 'one_hot':
            ...
            call_backs = []
            hist = model.fit(X_coord_train,y_train, validation_data=(X_coord_validation, y_validation),
            epochs=args.epochs,batch_size=args.bs, shuffle=args.shuffle, callbacks=[tf.keras.callbacks.ModelCheckpoint(args.model_folder+'best_weights.coord_'+episodes_involved+'.h5', monitor='val_loss', verbose=1, save_best_only=True,mode='auto'),tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=15, verbose=2, mode='auto')])
            ...
            print('***************Testing model************')
            model.load_weights(args.model_folder + 'best_weights.coord_'+episodes_involved+'.h5', by_name=True)   ## Restoring best weight for testing
            scores = model.evaluate(X_coord_test, y_test)
            print(model.metrics_names,scores)
        ...
        if args.strategy == 'one_hot':
            ...
            epochs=args.epochs,batch_size=args.bs, shuffle=args.shuffle, callbacks=[tf.keras.callbacks.ModelCheckpoint(args.model_folder+'best_weights.img_'+args.image_feature_to_use+'_'+episodes_involved+'.h5', monitor='val_loss', verbose=1, save_best_only=True,mode='auto'),tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=15, verbose=2, mode='auto')])
            ...
            print('***************Testing model************')
            model.load_weights(args.model_folder + 'best_weights.img_'+args.image_feature_to_use+'_'+episodes_involved+'.h5', by_name=True)
            scores = model.evaluate(X_img_test, y_test)
            print(model.metrics_names,scores)
        ...
        if args.strategy == 'one_hot':
            ...
            model.compile(loss=categorical_crossentropy,
                          ...
                          metrics=[metrics.categorical_accuracy,
                                        ...
                                        top_50_accuracy])
            model.summary()
            hist = model.fit(X_lidar_train,y_train, validation_data=(X_lidar_validation, y_validation),epochs=args.epochs,batch_size=args.bs, shuffle=args.shuffle,callbacks=[tf.keras.callbacks.ModelCheckpoint(args.model_folder+'best_weights.lidar_'+episodes_involved+'.h5', monitor='val_loss', verbose=2, save_best_only=True,mode='auto'),tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=15, verbose=2,mode='auto')])
            ...
            print('***************Testing model************')
            model.load_weights(args.model_folder + 'best_weights.lidar_'+episodes_involved+'.h5', by_name=True)   # to be added
            scores = model.evaluate(X_lidar_test, y_test)
            print(model.metrics_names,scores)
